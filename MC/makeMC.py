...
def getArgs() :
    ...
    parser.add_argument("-f","--inFile",default='MCsamples_2018UL.csv',help="Input file name.") 
...
for line in open(args.inFile,'r').readlines() :
    ...
    outLines.append("python ../../makeCondor.py --dataSet {0:s} --nickName {1:s} --mode {2:s} --year {3:s} -c {5:s} -s {4:s} -j {6:s} -l {7:s} -w 0\n".format(dataset,nickname, mode,era, args.selection, str(conc), args.doSystematics, str(args.islocal)))
