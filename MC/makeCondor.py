...
def beginBatchScript(baseFileName, Systematics) :
    ...
    outLines.append("export SCRAM_ARCH=el9_amd64_gcc11\n")
    outLines.append("eval `scramv1 project CMSSW CMSSW_13_0_10`\n")
    outLines.append("cd CMSSW_13_0_10/src\n")
    ...
    outLines.append("cd ${_CONDOR_SCRATCH_DIR}/CMSSW_13_0_10/src/\n")
    ...
    outLines.append("cd ${_CONDOR_SCRATCH_DIR}/CMSSW_13_0_10/src/DCH\n")
...
if 'pre' in whate or 'preV' in era or 'pre' in getNt or 'preV' in str(args.dataSet):
    ...
    eraD=eraD+'preVFP'
if str(args.islocal.lower())=='yes' : command ='ls  /eos/uscms/store/user/greddy/DCH_files/{0:s}/{1:s}/*root   > fileList.txt'.format(str(whate), getNt) 
if str(args.islocal.lower())=='yes' and executable == 'Gjets' : command ='ls  /eos/uscms/store/user/greddy/DCH_files/Gjets/{0:s}/{1:s}/*root   > fileList.txt'.format(str(whate), getNt) 
...
for nFile in range(0, len(dataset),mjobs) :
    ...
    outLines.append("cd ../.. ; rm -fr CMSSW_13_0_10\n")
...
wdir='/uscms_data/d3/greddy/CMSSW_13_0_10/src/'
dirMC = wdir+"/MC/"
dirCode = wdir+"/DCH/"
#dirData = wdir+"/data/"
funcsDir = wdir+"/funcs/"
